...
auto ExtractOrderFromQFMessage(FIX44::NewOrderSingle& order) -> bench_support::ParsedOrder {
...
// Precondition: sessions have been constructed (registered); responders are valid.
auto DoInProcessLogon(
    FIX::Session& initiator_session,
    ...
    // setResponder may call checkSessionTime; isNonStopSession must be true first.
...
auto RunQFSessionInboundBenchmark(
    ...
    // Declare responders before sessions so sessions (destroyed first in reverse order)
    // do not outlive their responders.
    BufferedResponder initiator_resp;
    ...
    // Pre-build application message frames. Seqnums start at 2 (logon used seqnum 1).
...
auto RunQFReplayBenchmark(
    ...
    // The acceptor received initiator seqnum 1 (logon), so it expects seqnum 2 next.
    // Each ResendRequest uses a consecutive seqnum.
    std::vector<std::string> resend_frames;
    resend_frames.reserve(iterations);
    for (std::uint32_t i = 0; i < iterations; ++i) {
        FIX44::ResendRequest resend_req(
            FIX::BeginSeqNo(2),
            FIX::EndSeqNo(static_cast<int>(replay_span) + 1));
...
public:
    ...
    void waitForAck(std::uint32_t expected) {
        std::unique_lock<std::mutex> lock(mutex);
        cv.wait(lock, [this, expected] {
            return ack_count.load(std::memory_order_acquire) >= expected;
        });
    }
    ...
    void fromApp(const FIX::Message& msg, const FIX::SessionID& session_id)
        ...
        if (msg_type.getValue() == "D") {
            ...
            FIX44::ExecutionReport exec_rpt(
                FIX::OrderID("ORDER-1"),
                FIX::ExecID("EXEC-1"),
                FIX::ExecType('0'),
                FIX::OrdStatus('0'),
...
int main(int argc, char** argv) {
    ...
    if (replay_iterations > 0U) {
        auto replay = RunQFReplayBenchmark(
            dictionary, xml_path, business_order, fixed_sending_time, replay_iterations, replay_span);
