...
//   Parent process: initiator — records T0, sends payload, recvs echo, records T1.
//   Sender fires at fixed rate (--send-interval-us); receive runs independently.
//
// Usage:
...
static int make_listener(std::uint16_t& out_port) {
...
// Full blocking send
static bool send_all(int fd, const std::uint8_t* buf, std::size_t n) {
    while (n > 0) {
        const ssize_t r = ::send(fd, buf, n, MSG_NOSIGNAL);
        if (r <= 0) return false;
        buf += r; n -= static_cast<std::size_t>(r);
    }
    return true;
}

// Full blocking recv
static bool recv_all(int fd, std::uint8_t* buf, std::size_t n) {
    while (n > 0) {
...
static bool busy_recv_all(int fd, std::uint8_t* buf, std::size_t n) {
    while (n > 0) {
...
static Stats compute(std::vector<std::uint64_t> v) {
    if (v.empty()) return {};
...
static void print_stats(const char* label, const Stats& s) {
    if (s.count == 0) { std::printf("  %-24s  no data\n", label); return; }
...
[[noreturn]] static void run_server(int listen_fd, std::size_t payload_bytes) {
    // Signal parent we're ready (via the listen socket existing); just accept.
    int conn = ::accept(listen_fd, nullptr, nullptr);
    ::close(listen_fd);
    if (conn < 0) { std::perror("[server] accept"); _exit(1); }
    int one = 1;
    ::setsockopt(conn, IPPROTO_TCP, TCP_NODELAY, &one, sizeof(one));

    std::vector<std::uint8_t> buf(payload_bytes);
    while (true) {
        if (!busy_recv_all(conn, buf.data(), payload_bytes)) break;
        if (!send_all(conn, buf.data(), payload_bytes)) break;
    }
    ::close(conn);
    _exit(0);
}
...
static std::vector<std::uint64_t> run_initiator(
        int fd,
    ...
    std::thread receiver([&] {
        ...
        while (done < total) {
            ...
            buf[0] = first;
            if (payload_bytes > 1) {
                if (!recv_all(fd, buf.data() + 1, payload_bytes - 1)) break;
            }
...
int main(int argc, char** argv) {
    ...
    if (payload_bytes < sizeof(std::uint32_t)) payload_bytes = sizeof(std::uint32_t);
    ...
    std::printf("TCP loopback RTT: %u iterations, %u warmup, %zu-byte payload,"
        ...
        iterations, warmup, payload_bytes, send_interval_us);
    ...
    if (all_rtts.size() > warmup) {
        all_rtts.erase(all_rtts.begin(),
                       all_rtts.begin() + static_cast<std::ptrdiff_t>(warmup));
    } else {
        all_rtts.clear();
    }
