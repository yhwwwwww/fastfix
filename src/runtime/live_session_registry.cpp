...
auto LiveSessionRegistry::Clear() -> void {
...
auto LiveSessionRegistry::ClearTerminalStatus() -> void {
    std::lock_guard lock(mutex_);
    terminal_status_.reset();
}

auto LiveSessionRegistry::HasActiveSession(std::uint64_t session_id) const -> bool {
    std::lock_guard lock(mutex_);
    return active_session_ids_.contains(session_id);
}

auto LiveSessionRegistry::RegisterActiveSession(std::uint64_t session_id) -> bool {
    std::lock_guard lock(mutex_);
    return active_session_ids_.emplace(session_id).second;
}

auto LiveSessionRegistry::UnregisterActiveSession(std::uint64_t session_id) -> void {
    std::lock_guard lock(mutex_);
    active_session_ids_.erase(session_id);
}

auto LiveSessionRegistry::SetTerminalStatus(base::Status status) -> void {
    std::lock_guard lock(mutex_);
...
auto LiveSessionRegistry::LoadTerminalStatus() const -> std::optional<base::Status> {
    std::lock_guard lock(mutex_);
    return terminal_status_;
}

auto LiveSessionRegistry::UpdateSnapshot(session::SessionSnapshot snapshot) -> void {
    std::lock_guard lock(mutex_);
    snapshots_[snapshot.session_id] = std::move(snapshot);
}

auto LiveSessionRegistry::LoadSnapshot(std::uint64_t session_id) const
...
auto LiveSessionRegistry::RegisterSubscriber(std::uint64_t session_id, std::size_t queue_capacity)
...
auto LiveSessionRegistry::HasSubscribers(std::uint64_t session_id) -> bool {
    std::lock_guard lock(mutex_);
...
auto LiveSessionRegistry::PublishNotification(const session::SessionNotification& notification) -> void {
