...
inline constexpr auto BuildDigitPairTable() -> std::array<std::array<char, 2>, 100> {
...
inline auto FormatUint32(char* buf, std::uint32_t value) -> std::size_t {
    if (value < 10U) {
...
inline auto FormatInt64(char* buf, std::int64_t value) -> std::size_t {
    if (value < 0) {
        buf[0] = '-';
