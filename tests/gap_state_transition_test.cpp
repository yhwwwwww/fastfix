...
// Written TDD-style: these tests capture the expected behavior for 4 known bugs.
//
// Bug 1: Gap-triggering message dropped (initiator & acceptor).
// Bug 2: ObserveInboundSeq doesn't auto-complete resend on normal seq increment.
// Bug 3: No PendingLogon timeout in OnTimer / NextTimerDeadline.
// Bug 4: Same as Bug 1 for acceptor side (covered by Bug 1 tests).
...
// Helper: create an initiator AdminProtocol, connect, and send logon (PendingLogon state).
// Returns the protocol and its outbound logon frame.
struct InitiatorSetup {
    fastfix::store::MemorySessionStore store;
    std::unique_ptr<AdminProtocol> protocol;
};

auto MakeInitiatorPendingLogon(
    const fastfix::profile::NormalizedDictionaryView& dictionary,
...
TEST_CASE("gap-transition: initiator logon response with gap activates after fill",
          "[gap-transition][bug1][initiator]") {
    ...
    fastfix::message::MessageBuilder logon_builder("A");
    logon_builder.set_string(35U, "A").set_int(98U, 0).set_int(108U, 30);
    auto logon_frame = EncodeInboundFrame(
        std::move(logon_builder).build(),
        dictionary.value(),
        "FIX.4.4",
        "SELL",
    ...
    auto resend_decoded = fastfix::codec::DecodeFixMessage(
...
TEST_CASE("gap-transition: acceptor logon with gap activates after fill",
          "[gap-transition][bug1][acceptor]") {
    auto dictionary = fastfix::tests::LoadFix44DictionaryView();
    if (!dictionary.ok()) {
        SKIP("FIX44 artifact not available: " << dictionary.status().message());
    }
...
TEST_CASE("gap-transition: app message triggering gap is delivered after fill",
          "[gap-transition][bug1][app-message]") {
    auto dictionary = fastfix::tests::LoadFix44DictionaryView();
    if (!dictionary.ok()) {
        SKIP("FIX44 artifact not available: " << dictionary.status().message());
    }
    ...
    std::size_t total_app_messages = gap_event.value().application_messages.size() +
                                     fill_event.value().application_messages.size();
...
TEST_CASE("gap-transition: logout triggering gap causes disconnect after fill",
          "[gap-transition][bug1][logout]") {
    auto dictionary = fastfix::tests::LoadFix44DictionaryView();
    if (!dictionary.ok()) {
        SKIP("FIX44 artifact not available: " << dictionary.status().message());
    }
    ...
    auto logout_frame = EncodeInboundFrame(
...
TEST_CASE("gap-transition: test request triggering gap gets heartbeat after fill",
    ...
    auto test_req_frame = EncodeInboundFrame(
        std::move(test_req_builder).build(),
        dictionary.value(),
        "FIX.4.4",
        "BUY",
        "SELL",
...
// ===========================================================================
// SessionCore level: after gap fill via individual message replay (not GapFill),
// ObserveInboundSeq should detect that next_in_seq has passed end_seq and
// auto-complete the resend.

TEST_CASE("gap-transition: ObserveInboundSeq auto-completes resend on normal increment",
...
TEST_CASE("gap-transition: replayed messages complete gap recovery",
          "[gap-transition][bug2][protocol]") {
    auto dictionary = fastfix::tests::LoadFix44DictionaryView();
    if (!dictionary.ok()) {
        SKIP("FIX44 artifact not available: " << dictionary.status().message());
    }
    ...
    auto hb_frame = EncodeInboundFrame(
        std::move(hb_builder).build(),
        dictionary.value(),
        "FIX.4.4",
        "BUY",
        "SELL",
...
TEST_CASE("gap-transition: OnTimer disconnects stale PendingLogon",
          "[gap-transition][bug3][timeout]") {
    auto dictionary = fastfix::tests::LoadFix44DictionaryView();
    if (!dictionary.ok()) {
        SKIP("FIX44 artifact not available: " << dictionary.status().message());
    }
...
TEST_CASE("gap-transition: second gap-triggering message extends gap then both delivered",
    ...
    auto fill_frame1 = EncodeInboundFrame(
        std::move(gap_fill1).build(), dictionary.value(),
        "FIX.4.4", "BUY", "SELL", 2U, true, {}, "20260414-11:59:00.000");
    ...
    auto fill_frame2 = EncodeInboundFrame(
        std::move(gap_fill2).build(), dictionary.value(),
        "FIX.4.4", "BUY", "SELL", 5U, true, {}, "20260414-11:59:02.000");
    ...
    std::size_t total_app = event1.value().application_messages.size() +
                            event2.value().application_messages.size() +
...
TEST_CASE("gap-transition: initiator reset-seq-num logon with gap",
          "[gap-transition][bug1][reset-seq]") {
    auto dictionary = fastfix::tests::LoadFix44DictionaryView();
    if (!dictionary.ok()) {
        SKIP("FIX44 artifact not available: " << dictionary.status().message());
    }
    ...
    auto fill_frame = EncodeInboundFrame(
        std::move(gap_fill).build(), dictionary.value(),
        "FIX.4.4", "SELL", "BUY", 1U, true, {}, "20260414-11:59:00.000");
